...
class Solution {
private:
    const std::vector<std::vector<int>> m_hops {{4,6}, {6,8}, {7,9}, {4,8}, {0,3,9}, {}, {0,1,7}, {2,6}, {1,3}, {2,4}};
    static constexpr int m_mod = 1000000007;

public:
    int knightDialer(int N) {
        std::vector<int> current(10, 1);
        for (std::vector<int> next(10, 0); --N > 0; std::for_each(next.begin(), next.end(), [](auto& n) { n = 0; })) {
            for (int i {0}; i <= 9; ++i) {
        ...
        return std::accumulate(current.cbegin(), current.cend(), 0) % m_mod;
    }
};
